...
var nameDefault = getPackageJsonValue('name', 'TSOA');
var versionDefault = getPackageJsonValue('version', '1.0.0');
var descriptionDefault = getPackageJsonValue('description', 'Build swagger-compliant REST APIs using TypeScript and Node');
var licenseDefault = getPackageJsonValue('license', 'MIT');
...
function swaggerSpecGenerator(args) {
    try {
        ...
        var metadata = new metadataGenerator_1.MetadataGenerator(swaggerConfig.entryFile, compilerOptions, config.ignore).Generate();
...
function routeGenerator(args) {
    try {
        ...
        var metadata = new metadataGenerator_1.MetadataGenerator(routesConfig.entryFile, compilerOptions, config.ignore).Generate();
